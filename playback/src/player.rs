                        ...
                        *stream_position_pcm = *stream_position_pcm + (packet.data().len() / 2) as u64;
                        ...
                        if notify_about_position {
                            *reported_nominal_start_time = Some(
                                Instant::now() - Duration::from_millis(stream_position_millis as u64),
                            );
...
impl PlayerInternal {
            ...
            if previous_track_id == track_id {
                ...
                if let Some(mut loaded_track) = loaded_track {
                    if Self::position_ms_to_pcm(position_ms) != loaded_track.stream_position_pcm {
                        loaded_track.stream_loader_controller.set_random_access_mode();
    ...
    fn load_track(
        ...
        std::thread::spawn(move || {
            loader.load_track(spotify_id, position_ms).and_then(move |data| {
    ...
    fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        match *self {
            PlayerCommand::Load {
                ...
                .field(&position_ms)
                .finish(),
            PlayerCommand::Preload { track_id } => f.debug_tuple("Preload").field(&track_id).finish(),
