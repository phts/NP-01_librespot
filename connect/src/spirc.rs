...
use core::spotify_id::{SpotifyAudioType, SpotifyId, SpotifyIdError};
...
impl SpircTask {
        ...
        match frame.get_typ() {
            ...
            MessageType::kMessageTypeLoad => {
                ...
                if self.state.get_track().len() > 0 {
                    let start_playing = frame.get_state().get_status() == PlayStatus::kPlayStatusPlay;
    ...
    fn handle_play_pause(&mut self) {
        match self.play_status {
            SpircPlayStatus::Paused { .. } | SpircPlayStatus::LoadingPause { .. } => self.handle_play(),
            SpircPlayStatus::Playing { .. } | SpircPlayStatus::LoadingPlay { .. } => self.handle_play(),
            _ => (),
        }
    }
    ...
    fn handle_unavailable(&mut self, track_id: SpotifyId) {
        ...
        for &index in unavailables.iter() {
            ...
            std::mem::swap(&mut self.state.mut_track()[index], &mut unplayable_track_ref);
            debug!(
    ...
    fn handle_unshuffle(&mut self) {
        // Reset tracks from context
        let uri = self.state.get_context_uri().to_owned();
        let page_url = format!("hm://context-resolve/v1/{}", uri);
        warn!("Attempting to resolve page {:?}", page_url);
        self.context_fut = self.resolve_uri(&page_url);
        // let response = self.session.mercury().get(uri).wait().unwrap();
    ...
    fn get_track_index_for_spotify_id(&self, track_id: &SpotifyId, start_index: usize) -> Vec<usize> {
    ...
    fn set_volume(&mut self, volume: u16) {
        ...
        let volume_to_mixer = volume_to_mixer(volume, &self.config.volume_ctrl);
